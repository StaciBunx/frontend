<template>
  <div>
    ...
    <svg class="projects__star" xmlns="http://www.w3.org/2000/svg" width="36" height="35" viewBox="0 0 36 35" fill="none">
      <g filter="url(#filter0_d_993_156)">
        <path @click="addToFavourite"
...
export default {
  ...
  methods: {
    addToFavourite (event) {
      if (event.target.classList.contains('added_to_fav')) {
        event.target.classList.remove('added_to_fav')
      } else event.target.classList.add('added_to_fav')
    }
  }
